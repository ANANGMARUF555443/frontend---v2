...
export default function ResultGame1({ hasil, sesi, onMainLagi, onExit }) {
  ...
  const durasiDetik = Math.max(
    0,
    Math.round(
      (new Date(hasil.finished_at).getTime() -
        new Date(hasil.started_at).getTime()) /
        1000
    )
  )
  ...
  return (
    <div className="game1-result">
      ...
      <div className="game1-result-score">
        ...
        <span className="game1-result-score-sub">
          {hasil.jumlah_benar} benar dari {total} soal
        </span>
      </div>
      ...
      {salahList.length > 0 && (
        <section className="game1-block">
          <h2 className="game1-block-title">
            Perlu diulang ({salahList.length})
          </h2>
          <div className="game1-review-list">
            {salahList.map((d) => {
              const soal = soalById[d.kosakata_id]
              if (!soal) return null
              const opsiBenar = soal.opsi.find(
                (o) => o.kosakata_id === d.jawaban_benar_kosakata_id
              )
              const opsiDipilih = soal.opsi.find(
                (o) => o.kosakata_id === d.id_kosakata_dipilih
              )
              return (
      ...
      <div className="game1-result-actions">
        <button type="button" className="game1-btn-primary" onClick={onMainLagi}>
          Main Lagi
        </button>
        <button type="button" className="game1-btn-secondary" onClick={onExit}>
          Kembali ke Quiz
        </button>
      </div>
    </div>
