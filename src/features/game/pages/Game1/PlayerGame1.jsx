...
// Menjalankan satu sesi soal Game 1. Timer per-soal berjalan mundur dari
// durasi_per_soal_detik; kalau habis waktu tanpa jawaban, otomatis lanjut
// (dianggap tidak dijawab). Semua jawaban dikumpulkan lokal, baru dikirim
// sekaligus ke backend lewat onSelesai saat soal terakhir selesai/dilewati.
export default function PlayerGame1({ sesi, onSelesai, onBatal }) {
  const totalSoal = sesi.soal.length
  const [indeks, setIndeks] = useState(0)
  const [dipilih, setDipilih] = useState(null) // kosakata_id opsi yang dipilih user di soal saat ini
  const [terkunci, setTerkunci] = useState(false) // true setelah user pilih jawaban, sebelum lanjut soal
  const [sisaDetik, setSisaDetik] = useState(sesi.durasi_per_soal_detik)
  const jawabanRef = useRef([]) // { kosakata_id, id_kosakata_dipilih, waktu_jawab_detik }
  ...
  useEffect(() => {
    if (terkunci) return
    if (sisaDetik <= 0) {
      lanjutKeSoalBerikutnya(null) // timeout = tidak dijawab
      return
    }
  ...
  function pilihJawaban(kosakataIdOpsi) {
    ...
    setTerkunci(true)
    // Beri jeda singkat supaya user melihat opsi mana yang benar/salah
  ...
  return (
    <div className="game1-player">
      <div className="game1-player-head">
        <button type="button" className="game1-exit-btn" onClick={onBatal}>&larr; Keluar</button>
        <span className="game1-progress-text">Soal {indeks + 1} / {totalSoal}</span>
      </div>
      ...
      <div className={`game1-opsi-grid game1-opsi-grid--${sesi.jumlah_pilihan_jawaban}`}>
